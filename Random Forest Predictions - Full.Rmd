...
rf_data <- clusters %>%
  unite(Predictor, Variable, Year) %>%
  spread(Predictor, value = Value) %>%
  dplyr::select(-PC_WHITE_2000, -PC_WHITE_2010, -PC_WHITE_2017) %>%
...
fwrite(pdp_df, "Results/Longitudinal Clustering/Partial Dependency Values from randomForest packages.csv")
```
