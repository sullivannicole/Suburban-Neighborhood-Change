...
```{r}
# Input to cluster must be a data.frame only (not a tibble as well) & cannot contain duplicates - make sure all observations are unique with unique IDs; ID (in this case Tract #) must be a character, NOT integer64 (from data.table package)
cld_snc_3 <- cld3d(cluster_full, timeInData = time_in_data)
...
# Take a look at 8 clusters
set.seed(410)
cluster_full$clusters <- getClusters(cld_snc_3, 8)
...
# fwrite(cluster_3_snc, "Results/Longitudinal Clustering/8-Clusters 3-Timepoints Inflation-Adjusted.csv")
```
