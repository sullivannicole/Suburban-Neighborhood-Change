...
writeOGR(snc_tracts_sp, "Data", "Tracts_ACS_Data_Prior_to_Rent_Interpolation", driver = "ESRI Shapefile")
```
...
arcpy.MakeFeatureLayer_management("N:/CommDev/Research/Research/Suburban-Neighborhood-Change/Project/Data/Tracts_ACS_Data_Prior_to_Rent_Interpolation.shp", "tracts_for_int_lyr")

arcpy.FillMissingValues_stpm("tracts_for_int_lyr", "Rent_Interpolated", "med_rnt", "AVERAGE",
...
snc_for_cluster <- snc_full %>% unique()
...
clusters_and_data %>%
  mutate(cluster = as.factor(cluster)) %>%
  ggplot(aes(median_hhi, perc_poc, color = cluster)) +
  geom_point() +
  scale_color_manual(values = c("#FF6666", "#FFCC66", "#99CCCC")) +
  labs(title = "Med. HHI v. % POC by Cluster") +
  theme(text = element_text(family = "Tw Cen MT"),
    plot.title = element_text(hjust = 0.5, size = 30),
        panel.background = element_rect(fill = "white"),
        legend.position = "bottom",
...
clusters_and_data %>%
  mutate(cluster = as.factor(cluster)) %>%
  ggplot(aes(perc_65up, perc_poc, color = cluster)) +
  geom_point() +
  scale_color_manual(values = c("#FF6666", "#FFCC66", "#99CCCC")) +
  labs(title = "Med. HHI v. % POC by Cluster") +
  theme(text = element_text(family = "Tw Cen MT"),
...
clusters_and_data %>%
  mutate(cluster = as.factor(cluster)) %>%
  ggplot(aes(median_hhi, perc_new_bld, color = cluster)) +
  geom_point() +
  scale_color_manual(values = c("#FF6666", "#FFCC66", "#99CCCC")) +
  labs(title = "Med. HHI v. % POC by Cluster") +
  theme(text = element_text(family = "Tw Cen MT"),
